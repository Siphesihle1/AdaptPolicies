...
def draw_bounding_box_on_image(
    ...
    for display_str in display_str_list[::-1]:
        text_width, text_height = (
            font.getbbox(display_str)[2],
            font.getbbox(display_str)[3],
        )
...
def draw_mask_on_image_array(
    image: np.ndarray, mask: np.ndarray, color="red", alpha=0.4
):
...
def visualize_boxes_and_labels_on_image_array(
    ...
    class_to_color_map: Optional[Dict[str, str]] = None,
    instance_masks: Optional[np.ndarray] = None,
    instance_boundaries: Optional[np.ndarray] = None,
    ...
    bounding_box_order: Tuple = (0, 1, 2, 3),
):
        ...
        if scores is None or scores[i] > min_score_thresh:
            ...
            if scores is None:
                ...
                if plot_color is not None:
                    ...
                    if class_to_color_map:
                        box_to_color_map[box] = class_to_color_map[labels[i]]
    ...
    for box, color in box_color_iter:
        ymin = box[bounding_box_order[0]]
        xmin = box[bounding_box_order[1]]
        ymax = box[bounding_box_order[2]]
        xmax = box[bounding_box_order[3]]
...
def expand_boxes(boxes: np.ndarray, scale: float):
...
def plot_mask(color: str, alpha: float, original_image: np.ndarray, mask: np.ndarray):
    rgb = ImageColor.getrgb(color)
...
def plot_detection_stats(
    ...
    for anno_idx in indices:
        rpn_score = roi_scores[anno_idx]
        bbox = boxes[anno_idx]
