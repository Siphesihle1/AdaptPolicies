...
def draw_mask_on_image_array(image: np.ndarray, mask: np.ndarray, color='red', alpha=0.4):
...
def visualize_boxes_and_labels_on_image_array(
  ...
  if box_to_score_map:
    box_color_iter = sorted(
      ...
      key=lambda kv: box_to_score_map[kv[0]]
    )
  else:
...
def plot_detection_stats(
  ...
  for anno_idx in indices:
    ...
    axs[3].set_yticklabels(
      category_names, fontdict={'fontsize': fontsize}
    )
