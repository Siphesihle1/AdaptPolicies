...
def nvidia_devices():
    ...
    for i in range(device_count):
        ...
        devices.append(pci_info.busId.decode() if hasattr(pci_info.busId, 'decode') else pci_info.busId)
...
"""
    ...
    for i, bus_id in enumerate(devices):
        xorg_conf.append(device_section.format(device_id=i, bus_id=nvml_busid_to_xorg(bus_id)))
...
def startx(display):
    ...
    try:
        ...
        command = shlex.split("Xorg -noreset +extension GLX +extension RANDR +extension RENDER -config %s :%s" % (rel_path, display))
