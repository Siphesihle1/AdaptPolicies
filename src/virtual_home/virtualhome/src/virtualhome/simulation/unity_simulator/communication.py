    ...
    def check_port(self, port_number):
        ...
        try:
            ...
            raise Exception(
                ...
                "The port {0} is already being used.".format(port_number)
            )
        finally:
            s.close()
        ...
        if launch_string is None:
            ...
            if not docker_training:
                ...
                subprocess_args += [
                    ...
                    "-logFile {}/Player_{}.log".format(
                        file_path, str(self.port_number)
                    ),
                ]
                ...
                try:
                    self.proc = subprocess.Popen(
                        subprocess_args, env=env, stdout=f, start_new_session=True
                    )
                    atexit.register(lambda: self.close)
                    # ret_val = self.proc.poll()
                except:
                    raise Exception(
                        "Error, environment was found but could not be launched"
                    )
