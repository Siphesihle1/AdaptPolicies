...
def world2im(camera_data, wcoords, imw, imh):
    ...
    pixelcoords = (pixelcoords + 1) / 2.0
    pixelcoords[1, :] = 1.0 - pixelcoords[1, :]
...
def find_edges_from(graph, id):
    nb_list = [
        (e["relation_type"], e["to_id"]) for e in graph["edges"] if e["from_id"] == id
    ]
...
def remove_edges(graph, n, fr=True, to=True):
    ...
    new_edges = [
        ...
        if (e["from_id"] != n_id or not fr) and (e["to_id"] != n_id or not to)
    ]
...
def remove_edge(graph, fr_id, rel, to_id):
    new_edges = [
        ...
        for e in graph["edges"]
        if not (
            e["from_id"] == fr_id and e["to_id"] == to_id and e["relation_type"] == rel
        )
    ]
    graph["edges"] = new_edges
...
def get_scene_cameras(comm, ids, mode="normal"):
    ...
    (ok_img, imgs) = comm.camera_image(
        cameras_select, mode=mode, image_width=640, image_height=360
    )
    return imgs
...
def display_scene_modalities(
    ...
    for mode_name in modalities:
        (ok_img, imgs) = comm.camera_image(
            cameras_select, mode=mode_name, image_width=640, image_height=320
        )
        if mode_name == "depth":
            imgs = [(x * 255.0 / np.max(x)).astype(np.uint8) for x in imgs]
...
def run_program(comm, prog, name):
    comm.render_script(
        prog, processing_time_limit=60, find_solution=True, file_name_prefix=name
    )
