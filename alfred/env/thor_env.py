...
class ThorEnv(Controller):
    ...
    def __init__(self, x_display=constants.X_DISPLAY,
                 ...
                 build_path=constants.BUILD_PATH, headless=True):
    ...
    def set_task(self, traj, args, reward_type='sparse', max_episode_length=2000):
        ...
        self.task = get_task(task_type, traj, self, args,
    ...
    def smooth_move_ahead(self, action, render_settings=None):
        ...
        new_action['moveMagnitude'] = constants.AGENT_STEP_SIZE / \
            smoothing_factor
    ...
    def to_thor_api_exec(self, action, object_id="", smooth_nav=False):
        ...
        if "RotateLeft" in action:
            ...
            if len(inventory_objects) == 0 or 'Knife' not in inventory_objects[0]['objectType']:
                raise Exception(
                    "Agent should be holding a knife before slicing.")
            ...
            raise Exception(
                "Invalid action. Conversion to THOR API failed! (action='" + str(action) + "')")
    ...
    def prune_by_any_interaction(self, instances_ids):
        ...
        ordered_instance_ids = [
            id for id in instances_ids if id in pruned_instance_ids]
    ...
    def va_interact(self, action, interact_mask=None, smooth_nav=True, mask_px_sample=1, debug=False):
        ...
        if type(interact_mask) is str and interact_mask == "NULL":
            ...
            instance_segs = np.array(
                self.last_event.instance_segmentation_frame)
            ...
            for color_id, intersection_count in instance_counter.most_common():
                union_count = np.sum(np.logical_or(
                    np.all(instance_segs == color_id, axis=2), interact_mask.astype(bool)))
                iou_scores[color_id] = intersection_count / float(union_count)
            iou_sorted_instance_ids = list(OrderedDict(
            ...
            # print instance_ids
            instance_ids = [
                inst_id for inst_id in all_ids if inst_id is not None]
            if debug:
        ...
        if debug:
            print("taking action: " + str(action) +
                  " on target_instance_id " + str(target_instance_id))
        try:
